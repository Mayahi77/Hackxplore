...
**Progressive Disclosure UX Pattern (Preview-before-detail):**
AI responses render as a list of collapsed hypothesis cards.

Each card has two states:

| State | Content |
|-------|---------|
| Collapsed (default) | One-line summary, e.g. *"Lubrication failure — oil starvation under load"* |
| Expanded (tap to open) | Full detail: root cause explanation, troubleshooting steps, linked sources, diagrams |

Default: all cards collapsed. Engineer scans the list, taps only relevant cards.
This matches the confirmed user need: rough overview first, detail on demand — not a 20-page manual before the engineer can assess relevance.
...
> **Scope: Optional / post-MVP.** No direct JTBD from user research drives standalone graph navigation. In MVP, problem context is surfaced through the problem worksheet (§5.10) only. Standalone graph deferred.

The platform can show a contextual, problem-linked knowledge structure for the active problem:
...
This would serve as a visual complement to the worksheet. Not a standalone navigation feature — engineers see only what is relevant to their current problem.
...
### 5.12 Colleague Collaboration

When engineers get stuck, they need to share problems with colleagues without re-explaining everything. The system supports three collaboration modes on any problem worksheet.

**Share worksheet (read-only)**
- Generate a shareable link to the current worksheet
- Recipient sees full problem state: description, hypotheses, steps tried with outcomes
- No account required to view [Assumption: link-based access; auth model TBD]

**Add collaborator / advisor**
- Invite a colleague by name or role within the platform
- Collaborator can:
  - add comments on specific steps or hypotheses
  - suggest additional troubleshooting paths
  - mark their own observations inline
- Async — both engineers do not need to be online simultaneously
- Notification sent to invited collaborator

**Direct contact**
- Send message: opens in-app message with worksheet link attached as context
- Call: one-tap call trigger using device native phone/dialer

This resolves the JTBD: *"When I'm stuck, I want to share my problem with a colleague without re-explaining."*
...
- progressive disclosure UI: collapsed hypothesis cards, expand on tap
...
- worksheet sharing (read-only link + add collaborator)
- mobile-first

### Optional MVP Features

- contextual knowledge view (problem-linked graph — deferred, no core JTBD)
- expert user rating and flagging system
- author notification on incorrect flag
- in-app messaging and call trigger for colleague collaboration
- gamification — deprioritized (no interview signal, no JTBD)
